...
为确保 HUD 持续符合“严格 MVP / 少而稳”的目标，本规格对常驻信息数量、屏幕占用和可遮挡区域设置硬预算。

| 项目 | 预算 | 说明 |
|---|---|---|
| 常驻 HUD 元素总数 | ≤ 8 | 仅允许：日期/赛季、经费、运动点数、行动窗口、下一场比赛状态、菜单、球员入口、比赛入口 |
| 顶部状态栏高度 | 48px（安全范围 44–56px） | 服从 `Tuning Knobs` |
| 底部导航高度 | 56px（安全范围 48–64px） | 服从 `Tuning Knobs` |
| 顶部+底部 HUD 总占屏高度 | ≤ 屏幕高度 15% | 常规 16:9 分辨率下的硬上限 |
| 主内容区保留高度 | ≥ 屏幕高度 85% | 保证 Home / Roster / Training / Schedule 的核心内容不被压扁 |
| 核心视觉区 | 屏幕中央 60% 宽 × 50% 高 | 常驻 HUD、常驻提醒、未来扩展入口不得覆盖此区 |

### Budget Rules

- HUD 的常驻视觉预算只覆盖顶部状态栏和底部导航，不包括临时出现的 Overlay。
- Toast、Tooltip、局部错误提示如需出现，默认应避开核心视觉区；若因极端分辨率不可避免，也不得长期停留。
- 比赛流程接管（Match Pre / Match Live / Match Result）时，HUD 必须完整隐藏，不得保留任何常驻元素占用预算。
- 任何未来新增常驻元素若会突破“8 项上限”或“15% 占屏上限”，必须先回到 HUD 文档修订，而不能直接在实现中追加。
...
- [ ] HUD 常驻元素总数不超过 8 项；顶部状态栏与底部导航总占屏高度不超过屏幕高度 15%；屏幕中央 60% 宽 × 50% 高的核心视觉区不被常驻 HUD 覆盖。
