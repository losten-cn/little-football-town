...
2. 本系统只拥有"球员信息如何呈现"层面的规则，不拥有球员属性定义、成长计算、训练逻辑或状态判定：
...
20. MVP 阶段球员管理 UI 的首要目标是验证：玩家在 Roster 和 Player Detail 两个屏幕内能否完整回答"我有哪些球员、谁值得培养、谁已经到顶、下一步该练谁"。
...
| Detail View | 单名球员详情展示 | Player Detail | 查看五维属性、查看成长轨迹、查看状态、点击训练入口（携带球员上下文）、返回 Roster View |
...
> **Provisional assumption:** 技能与特性系统尚未设计完成，详情页中与技能/特性相关的展示区域预留位置但 MVP 暂不实现具体内容。
...
`detail_section_order = [identity, attributes, growth, status, actions]`
...
| 技能与特性系统 | Soft (provisional) | 详情页需预留技能/特性展示区域，但该系统尚未设计 | 技能/特性内容——当前预留展示位置，使用占位状态 |
...
3. 当技能与特性系统 GDD 完成后，本节相关 provisional 条目应从 Soft 升级为 Hard，并在该 GDD 的 Dependencies 中反向声明对本系统详情页展示区域的依赖。
4. 运动员培养系统拥有球员数据的完整语义；如果培养系统新增或修改了球员字段，本系统必须同步更新对应的可视化规则。
...
- **GIVEN** 球员详情页已加载，**WHEN** QA 检查信息区域，**THEN** 必须按顺序展示：基础身份区 → 五维属性区 → 成长信息区 → 状态区 → 操作入口区。
