...
4. 玩家在赛前必须至少完成三类准备：选择可出场球员、确定首发/位置安排、确认本场战术方案。若玩家不主动细调，系统必须提供可直接采用的推荐阵容与推荐战术，避免比赛入口成为高门槛阻塞点。比赛表现 UI 在自由赛前编辑态中可以阻止玩家用非法阵容手动开赛，但一旦正式比赛节点进入不可跳过的强制触发态，UI 不得阻塞本系统继续执行推荐阵容、错位补位或 `forfeit_result_packet` 兜底。
