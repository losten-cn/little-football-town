...
14. 经济管理系统已完成 MVP 设计；引导中涉及资源摘要的部分应简短说明经费和运动点数的用途，并明确研究点数在 MVP 阶段不展示、不解释。
15. MVP 阶段新手引导系统的首要目标是验证：一个完全不了解《足球小镇》的新玩家，能否在首次游戏会话中不借助外部帮助，完成"看球队 → 练一次 → 踢一场 → 看结果 → 回到主页"的完整闭环，并在结束时能用自己的话说出这个游戏在玩什么。
