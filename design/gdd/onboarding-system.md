...
4. MVP 引导必须覆盖以下最小概念集，按顺序引导：
   ...
   - 首场比赛——赛前：如何查看对手摘要、接受或轻看推荐阵容、开始比赛；
...
16. First Match Pre 引导只能高亮低压力最小路径：对手摘要、推荐阵容确认区、开赛确认按钮。战术选择、设施加成、技能/特性快照、联赛 stakes 和成长预期可以作为可选说明或赛后解释，不得在首场比赛前全部作为必须阅读或必须操作的教学点。
17. MVP 阶段新手引导系统的首要目标是验证：一个完全不了解《足球小镇》的新玩家，能否在首次游戏会话中不借助外部帮助，完成"看球队 → 练一次 → 踢一场 → 看结果 → 回到主页"的完整闭环，并在结束时能用自己的话说出这个游戏在玩什么。
...
- **GIVEN** 引导在 First Match Pre 步骤且 `match_trigger_reached = true`，**WHEN** QA 检查赛前界面，**THEN** 引导高亮只指向对手摘要、推荐阵容确认区和开赛确认按钮；不得同时要求玩家学习战术、设施加成、技能/特性快照、联赛 stakes 和成长预期。
