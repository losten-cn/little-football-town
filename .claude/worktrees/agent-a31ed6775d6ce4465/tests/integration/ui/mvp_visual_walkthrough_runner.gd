...
func _run() -> void:
	...
	_emit_training_blocked_payloads()
	await _capture("05_training_blocked", "training")

	_emit_player_payloads()
	await _capture("06_training_recovered", "training")

	await _press_button_and_wait("TrainingConfirmButton", "确认训练")
	await _capture("07_training_result", "home")
	await _capture("08_home_after_training", "home")

	_emit_match_ready_payloads(false)
	await _press_button_and_wait("PrimaryAction", "进入比赛")
	await _capture("09_home_match_disabled_reason", "home")

	_emit_match_ready_payloads(true)
	await _press_button_and_wait("PrimaryAction", "进入比赛")
	await _capture("10_match_pre", "match_pre")

	await _press_button_and_wait("PreMatchStartButton", "开始比赛")
	await _capture("11_match_live_empty", "match_live")

	_emit_event("match_event_occurred", {"minute": 15, "summary": "High 完成一次射门"})
	_emit_event("match_event_occurred", {"minute": 45, "summary": "0-0 中场"})
	await _capture("12_match_live_timeline", "match_live")
	...
	await _capture("13_match_result", "match_result")

	await _press_button_and_wait("ResultConfirmButton", "确认并返回 Home")
	await _capture("14_home_final", "home")
...
func _emit_player_payloads() -> void:
	_emit_event("roster_updated", {
		"players": [
			...
			{"id": 2, "name": "High", "position": "FW", "rating": 77, "development_tier": "重点", "status_tag": "可训练", "recent_growth": "+2", "attributes": {"SPD": {"current": 81, "potential": 88}, "PWR": {"current": 62, "potential": 70}, "TEC": {"current": 79, "potential": 85}, "INT": {"current": 68, "potential": 74}, "STA": {"current": 72, "potential": 72}}},
		],
	})
	_emit_event("training_options_updated", {
		...
		"options": [
			...
			{"training_id": "stamina", "name": "体能恢复", "summary": "恢复体能", "cost_summary": "运动点数 1", "available": false, "disable_reason": "运动点数不足"},
		],
	})


func _emit_training_blocked_payloads() -> void:
	_emit_event("training_options_updated", {
		"training_available": false,
		"training_disable_reason": "本轮训练次数已用完",
		"options": [
			{"training_id": "finishing", "name": "射门训练", "summary": "提升终结效率", "cost_summary": "经费 100｜运动点数 1", "available": false, "disable_reason": "本轮训练次数已用完"},
			{"training_id": "stamina", "name": "体能恢复", "summary": "恢复体能", "cost_summary": "运动点数 1", "available": false, "disable_reason": "本轮训练次数已用完"},
		],
	})
