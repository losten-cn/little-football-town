...
func apply_snapshot(snapshot: Dictionary[String, Variant]) -> void:
...
## Returns whether formal match entry is legal right now.
func can_enter_formal_match() -> bool:
	return get_match_trigger_reached()
...
## Returns a serializable authoritative state snapshot.
...
func get_hud_payload() -> Dictionary[String, Variant]:
	var date_text: String = _current_date_display
...
func _apply_snapshot(snapshot: Dictionary[String, Variant]) -> void:
