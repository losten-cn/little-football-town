...
**Output:** Updated story file (Status: Complete) + decision log + surfaced next story.
...
## Phase 7b: Decision Log and Archive

After Phase 7 updates are approved and written, create a structured decision log
for the completed story. This phase adds change traceability inside the existing
workflow; it does not replace the story file, GDD, ADR, or test evidence.

1. **Read the decision log template**
   - Read `.claude/templates/CHANGELOG_TEMPLATE.md`.
   - Replace every `{{PLACEHOLDER}}` with concrete information from this
     `/story-done` run.
   - Do not leave placeholders, `TBD`, or empty table rows in the final draft.

2. **Draft the decision log**
   - Include the story ID, title, epic, completion date, lead agent, review
     agent, summary of changes, files changed, affected systems, data/config
     changes, passed tests, code review notes, predecessor stories, and successor
     stories.
   - The **Design Decisions** table is mandatory. Record each key decision,
     the chosen option, the rationale, and at least one rejected alternative.
   - If the story does not contain explicit alternatives, infer them from the
     design discussion, GDD, ADR, implementation notes, or deviations. If no
     defensible alternative exists, write `None recorded` and flag this as an
     advisory traceability gap in the final response.
   - Use `game-designer` for design-heavy stories and `lead-programmer` for
     implementation-heavy stories to review the decision log draft for accuracy
     when review mode is `full`. In `lean` or `solo` mode, perform the review
     inline and note that no subagent review was run.

3. **Ask before writing**
   - Present the full decision log draft to the user.
   - Ask exactly: `May I write this to design/decisions/[story-id]-[slugified-title].md and update design/decisions/INDEX.md?`
   - If the user does not approve, stop Phase 7b. Do not write the decision log
     or update the index.

4. **Save the decision log**
   - File name format: `design/decisions/[story-id]-[slugified-title].md`.
   - Slug rules: lowercase title, ASCII letters/numbers where possible,
     spaces converted to hyphens, punctuation removed.
   - Write only the user-approved content.

5. **Update the decision index**
   - Read `design/decisions/INDEX.md`; if it is missing, create it with the
     standard decision-index headings and empty tables before inserting the
     new story row.
   - Insert the new story row at the top of the `## 最新变更` table, directly
     under the table separator:
     `| [Story ID] | [Title] | [YYYY-MM-DD] | [filename](./filename) |`
   - Update `## 按代理查看` with the lead agent and review agent where known.
   - Update `## 按系统查看` with affected systems from the decision log.
   - Preserve existing index rows and links.

6. **Archive and workspace check**
   - If related design sketches or temporary planning files exist and the user
     explicitly approves moving them, archive them under `design/archive/`.
   - Check for active worktrees or unmerged branches and report them. Do not
     merge, delete, or discard worktrees unless the user explicitly asks.
...
- **Never write a decision log or update `design/decisions/INDEX.md` without
  user approval** — Phase 7b requires the full draft to be shown first.
